...
class SWRDetector:
    ...
    def validate_swr_event(self, event: SWREvent,
                          control_power: float) -> Tuple[bool, str]:
        """
        Validate SWR candidates using the pyramidal channel only.

        Criteria:
        - Mean frequency > 100 Hz
        - At least 4 complete ripple cycles
        - Ripple power > 2x control band power
        """
        notes = []
        is_valid = True
    ...
    def detect_swr_events(self, pyramidal_channel: np.ndarray) -> List[SWREvent]:
        """
        Full SWR detection pipeline using the pyramidal channel directly.

        Args:
            pyramidal_channel: LFP signal from the selected pyramidal channel

        Returns:
            List of SWREvent objects
        """
        analysis_signal = pyramidal_channel
        ...
        for onset, peak, offset in events_boundaries:
            ...
            is_valid, notes = self.validate_swr_event(event, control_power)
        ...
        if total_events > 0:
            ...
            for i, e in enumerate(events[:10], 1):
                ...
                print(
                    ...
                    f"cycles={e.num_cycles:.1f}, ripple_pow={e.ripple_power:.4f}, "
